...
class PIDController(Controller):
    ...
    def set_next_reference(self):
        reference = self.trajectory.get_position_at((self.i + 1))  # DELTA_T = 0.1
        # reference = self.trajectory.get_position_at((self.i + 1) * self.delta)  # DELTA_T = 0.1
        self.x_ref_n_plus_1 = reference.x
        self.y_ref_n_plus_1 = reference.y

    def compute_control_actions(self, pose, twist, i):  # 主要是这一个方法
        self.i = i
        ...
        self.v_c_n = self.v_c_nm1 + a_v * self.e_v_n + b_v * self.e_v_nm1 + c_v * self.e_v_nm2  # TODO: unknown
    ...
    def compute_linear_speed_reference(self):
        ...
        v_ref_n = (v_x_ref ** 2 + v_y_ref ** 2) ** 0.  # 和速度标量
        return v_ref_n
